...
(function ($) {
  ...
  function hoverImage($el = $(document)) {
    ...
    itemWrap.each(function (index, item) {
      ...
      $(item)
        .find(".dsn-style-hover")
        .each(function (i, item) {
          const effect = new motionHoverEffect[
            "HoverImgFx" + (animateStyle || "1")
          ](item);
          dsnGrid.killAjax(() => {
            effect.kill();
          });
  ...
  function marquee($el = $(document)) {
    ...
    $el.find(".dsn-marquee").each((index, item) => {
      const marquees = $(item).marquee(
        dsnGrid.getData(item, "option", {}) || {}
      );
      dsnGrid.killAjax(() => {
        marquees === null || marquees === void 0
          ? void 0
      ...
      swiper: function ($id, $obj) {
        ...
        const renderBullet = swiperPaginate.hasClass("dsn-swiper-img")
          ? function (index, className) {
              ...
              return (
                ...
                className +
                '">' +
                ((_this$slides$index$qu =
                  this.slides[index].querySelector("img")) === null ||
                _this$slides$index$qu === void 0
                  ? void 0
                  : _this$slides$index$qu.outerHTML) +
                "</span>"
              );
            }
          : null;
        $obj = $.extend(
          true,
          {
            ...
            navigation: {
              nextEl: $id
                .find(".next-paginate ,.next-container", ".dsn-nav-right")
                .get(0),
              prevEl: $id
                .find(".prev-paginate , .prev-container", ".dsn-nav-left")
                .get(0),
            },
          },
          $obj
  ...
  const activeNav = function ($index) {
    ...
    (_prev$get = prev.get($index)) === null || _prev$get === void 0
      ? void 0
      : _prev$get.classList.add("dsn-active");
    (_next$get = next.get($index)) === null || _next$get === void 0
      ? void 0
      : _next$get.classList.add("dsn-active");
  };
  ...
  function sliderPortfolio($el = $(document)) {
    ...
    const initSlider = async function () {
      ...
      await this.querySelectorAll(".content-slider .slide-item").forEach(
        ($item, $index) => {
          ...
          const pushItem = `<div class="dsn-slide d-flex align-items-center justify-content-center">${img.prop(
            "outerHTML"
          )}<h6 class="sm-title-block">${
            (_slideContent$querySe =
              slideContent.querySelector(".title-lg")) === null ||
      ...
      $(this)
        .find(".dsn-nex-prev .prev .dsn-wrapper")
        .html(pagination.prev.join(""));
      $(this)
        .find(".dsn-nex-prev .next .dsn-wrapper")
        .html(pagination.next.join(""));
      ...
      await this.querySelectorAll(".dsn-slider-content .slide-content").forEach(
        function ($item) {
          dsnGrid.spltting.Items(
            $item,
    ...
    function progressBar(swiper, newNum) {
      gsap.to(this.querySelector(".progress-nav .progress-w-after"), 1, {
        width:
          ((Number.parseInt(newNum) + 1) / swiper.slides.length) * 100 + "%",
      });
    }
    ...
    function slideChange(swiper) {
      ...
      const [newNum, oldNum, newContent, oldContent] = getContent(
        ...
        [newTitle, oldTitle] = [
          Array.from(newContent.querySelectorAll(".title-lg .char") || []),
          Array.from(oldContent.querySelectorAll(".title-lg .char") || []),
        ],
      ...
      initSlider
        ...
        .then(
          function (swiper) {
            ...
            const elementNextPrev = $(this).find(
              ".dsn-nex-prev .prev , .dsn-nex-prev .next"
            );
